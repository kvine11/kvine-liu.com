...
- **One hover gesture, everywhere something is clickable:** a 12px indent plus one thing brightening, over 150ms `ease-out`. On an FRC link row the year goes `ink/48` → `ink/75`; on a linked entry title the arrow goes 55% → full. Same distance, same duration, same easing — a linked title and a link row are the same gesture and should feel like it. Don't give one of them its own motion.
- **Years live on the entry, at the right of the title row** — every entry carries a `year` (mono, 11px, `+0.16em`, `ink/48`), a single year or a range ("2024 — 2026"). Band headings carry no year range; they used to, and the per-entry year replaced it so dates read the same way everywhere regardless of what else an entry has. Don't put a year back in `meta` or on a band heading.
- **The `meta` line** (mono, uppercase, `ink/48`) under the title holds context: org · role, two slots. It's what lets an entry stand with no links at all, which is the normal case in the research band. It's omitted when there's no org to name — a personal project doesn't say "Personal Project", because the band already says that (Calorie Tracker). Don't add a third slot — where a fact doesn't fit, swap it against the one it beats (UTD dropped "TAST STEM-BRIDGE" to carry "Advised by Prof. Yanwen Xu").
