...
- `src/data/site.js` — **all page copy and links.** Editing the site's content means editing this file, not the components. `bands` is designed to grow: add an entry with `title`/`year`/`meta`/`description`/`tags` (and optionally `href` or `links`) to either band and it renders automatically, no component changes needed. Numbering is derived from position, so inserting an entry never means renumbering the ones after it.
- `public/favicon.svg` and `public/apple-touch-icon.png` — the tab icon: a **"K" monogram**, ink (`#f4f4f4`) strokes on a rounded ground tile (`#0f0f0f`), drawn as three paths so it needs no webfont. The tile is dark in both themes on purpose — a browser tab sits in the *browser's* chrome, not the site's, and one dark tile reads on light and dark chrome alike, so there's no second variant to keep in sync. This is not a break from "no logo, no graphic marks": like the theme switch, a letterform is a wordmark rather than a picture, and a tab icon is required chrome — the alternative isn't "nothing", it's the browser's default globe. The PNG is the same art at 180px for iOS home screens; re-render it from the SVG (`qlmanage -t -s 180`) if the mark ever changes.
...
- **There is no `--color-accent`.** Muted text, dividers, and hover states are all the ink token at reduced alpha — `text-ink/65`, `text-ink/45`, `border-ink/40` on hover. If a change seems to call for introducing a color, it almost certainly doesn't — reach for a different opacity step instead. Two named steps are the exception: `text-label` (entry numbers, years, meta lines, the eyebrow, "Say hi") and `text-nav` (the section links). They're still ink at an alpha, but the alpha differs per theme: 48% and 60% on dark, 65% and 68% on cream. That's because `ink/48` reads about 3:1 on cream, which fails AA for 11px text. Use the tokens for that text rather than `ink/NN`.
...
- **One hover gesture, everywhere something is clickable:** a 12px indent plus one thing brightening, over 150ms `ease-out`. On an FRC link row the year goes `text-label` → `ink/75`; on a linked entry title the arrow goes 55% → full. Same distance, same duration, same easing — a linked title and a link row are the same gesture and should feel like it. Don't give one of them its own motion.
- **Years live on the entry, at the right of the title row** — every entry carries a `year` (mono, 11px, `+0.16em`, `text-label`), a single year or a range ("2024 — 2026"). Band headings carry no year range; they used to, and the per-entry year replaced it so dates read the same way everywhere regardless of what else an entry has. Don't put a year back in `meta` or on a band heading.
- **The `meta` line** (mono, uppercase, `text-label`) under the title holds context: org · role, two slots. It's what lets an entry stand with no links at all, which is the normal case in the research band. It's omitted when there's no org to name — a personal project doesn't say "Personal Project", because the band already says that (Calorie Tracker). Don't add a third slot — where a fact doesn't fit, swap it against the one it beats (UTD dropped "TAST STEM-BRIDGE" to carry "Advised by Prof. Yanwen Xu").
...
- **Two typefaces, by role.** Schibsted Grotesk (`font-sans`) for everything readable, including the band headings; JetBrains Mono (`font-mono`) for the small uppercase labels only — eyebrow, entry numbers, entry and link years, `meta` context lines, tag chips, "Say hi", "In progress". This is a utility/caption face, not decoration, which is why it survived the move to text-only.
...
Two grounds, one system. Dark is the default; cream is `:root[data-theme="light"]` overriding the same three tokens in `index.css`. **It is not a second palette** — still ink-on-ground at varying alpha, still no accent, and every `ink/NN` in the codebase works unchanged in both. The only per-theme alphas are the `label` and `nav` tokens, which carry more ink on cream so small text clears 4.5:1 on the paler ground. Adding a color for one mode would break both.
...
- Both bands are full. Adding an entry is a `site.js` edit: `title`, `year`, `meta` (if there's an org), `description`, `tags`, and `href`/`links` only if there's something public to link. `PlaceholderEntry` still renders any entry without a `title`, so an open slot is `{}`.
